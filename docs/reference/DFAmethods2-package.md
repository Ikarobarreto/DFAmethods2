# DFAmethods2: detrended fluctuation analysis and related methods

A toolbox of Detrended Fluctuation Analysis ('DFA') methods for
long-range correlations, cross-correlations and regression in
nonstationary time series. See
[`vignette("DFAmethods2")`](https://ikarobarreto.github.io/DFATools/articles/DFAmethods2.md)
for the theory and worked examples.

## Function groups

- Scaling of a single series:

  [`dfa`](https://ikarobarreto.github.io/DFATools/reference/dfa.md),
  [`plotdfa`](https://ikarobarreto.github.io/DFATools/reference/plotdfa.md)

- Cross-correlation:

  [`rhodcca`](https://ikarobarreto.github.io/DFATools/reference/rhodcca.md),
  [`plotrdcca`](https://ikarobarreto.github.io/DFATools/reference/plotrdcca.md),
  [`plotdcca`](https://ikarobarreto.github.io/DFATools/reference/plotdcca.md)

- Partial cross-correlation:

  [`rhodpcca`](https://ikarobarreto.github.io/DFATools/reference/rhodpcca.md)

- Multiple cross-correlation:

  [`dmc2`](https://ikarobarreto.github.io/DFATools/reference/dmc2.md)

- Detrended fractal regression:

  [`betadfa`](https://ikarobarreto.github.io/DFATools/reference/betadfa.md),
  [`sbdfa`](https://ikarobarreto.github.io/DFATools/reference/sbdfa.md),
  [`fracreg`](https://ikarobarreto.github.io/DFATools/reference/fracreg.md),
  [`effsizeDFA`](https://ikarobarreto.github.io/DFATools/reference/effsizeDFA.md)

- Significance tests:

  [`fracreg.PStest`](https://ikarobarreto.github.io/DFATools/reference/fracreg.PStest.md),
  [`fracreg.Ktest`](https://ikarobarreto.github.io/DFATools/reference/fracreg.Ktest.md),
  [`fracreg.IUTest`](https://ikarobarreto.github.io/DFATools/reference/fracreg.IUTest.md)

## See also

Useful links:

- <https://github.com/Ikarobarreto/DFAmethods2>

- Report bugs at <https://github.com/Ikarobarreto/DFAmethods2/issues>

## Author

**Maintainer**: Ikaro Daniel de Carvalho Barreto
<daniel.carvalho.ib@gmail.com>

Authors:

- Luiz Henrique Dore
